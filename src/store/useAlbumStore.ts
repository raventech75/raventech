...
export type Page = {
  id: string;
  items: (PhotoItem|TextItem)[];
  /** dataURL de la miniature (facultatif). */
  preview?: string | null;
};
...
export type AlbumState = {
  ...
  pageCount: number;
  currentIndex: number;
  setPagePreview: (index:number, dataUrl:string|null)=>void;
  ...
  // **NOUVEAU**: reflow global après ajout/suppression pour que tout tienne
  reflowFitAll: (preferCols?: 1|2|3|4 | 'auto')=>void;
...
export const useAlbumStore = create<AlbumState>((set,get)=>({
  ...
  // —— Pages
  pages: [{ id: nanoid(), items: [], preview: null }],
  pageCount: 10,
  currentIndex: 0,
  setPagePreview: (index, dataUrl)=> set(s=>{
    if (index<0 || index>=s.pages.length) return s;
    const pages = s.pages.slice();
    pages[index] = { ...pages[index], preview: dataUrl };
    return { pages };
  }),
  ...
  setPageCount: (n)=> set(s=>{
    ...
    while (pages.length<target) pages.push({ id: nanoid(), items: [], preview:null });
    return { pageCount: target, pages, currentIndex: Math.min(s.currentIndex,pages.length-1) };
  }),
  addPage: ()=> set(s=>{
    if (s.pages.length>=25) return s;
    return { pages: [...s.pages,{id:nanoid(),items:[],preview:null}], pageCount: s.pageCount+1, currentIndex: s.pages.length };
  }),
  ...
  addAssets: async (files) => {
    ...
    // Import **ultra fluide** : on découpe par idle-callback si dispo
  ...
  // —— Placement auto (préserve l’homothétie via ar)
  placePhotoAuto: (assetId, baseTargetW)=> set(s=>{
    ...
    let w = safeDim(baseTargetW ?? (W-2*bleed)*0.45, 40);
    if (w > (W-2*bleed)) w = (W-2*bleed);
    let h = Math.round(w / ar);
    if (h > (H-2*bleed)) { h = (H-2*bleed); w = Math.round(h * ar); }

    const page = s.pages[s.currentIndex];
    // placement initial grossier en haut-gauche (reflow global ensuite)
    ...
    // ✅ Reflow global pour que tout tienne dans la page, sans chevauchement
    const tmp = { ...s, pages, assets } as AlbumState;
    const colsAuto = clamp(Math.round(Math.sqrt(pages[s.currentIndex].items.filter(i=>(i as any).kind==='photo').length)),1,4) as 1|2|3|4;
    (tmp as any).reflowFitAll = get().reflowFitAll; // hack local pour réutiliser l'algo
  ...
  placePhoto: (assetId, x=null, y=null, targetW=null)=> {
    ...
    set(s=>{
      ...
      const photos = pages[s.currentIndex].items.filter((i:any)=>i.kind==='photo') as PhotoItem[];
      const cols = colsAuto, rows=Math.ceil(photos.length/cols);
      ...
      photos.forEach((it,idx)=>{
        ...
        if (ar2>arBox){ w2=Math.round(cell.w); h2=Math.round(w2/ar2); }
        else { h2=Math.round(cell.h); w2=Math.round(h2*ar2); }
        it.width=w2; it.height=h2; it.x=Math.round(cell.x+(cell.w-w2)/2); it.y=Math.round(cell.y+(cell.h-h2)/2);
      ...
      return {
        ...
        items: pg.items.map((it:any)=>{
          ...
          if (it.kind==='photo') {
            ...
            // width/height monodirectionnels -> on recalcule l’autre
  ...
  clearPage: (pageId)=> set(s=>{
    ...
    set((s) => {
      const pages = s.pages.map((pg) => {
        ...
        if (idx < 0) return pg;
        const next = pg.items.slice();
        const [it] = next.splice(idx, 1);
        next.push(it); // dessiné en dernier => au-dessus
        return { ...pg, items: next };
      });
      return { pages };
    }),
  ...
  sendToBack: (pageId, itemId) =>
    set((s) => {
      const pages = s.pages.map((pg) => {
        ...
        const next = pg.items.slice();
        const [it] = next.splice(idx, 1);
        next.unshift(it); // dessiné en premier => en-dessous
        return { ...pg, items: next };
      });
      return { pages };
    }),
  ...
  // —— Layouts simples (grille & mosaïque)
  ...
  // —— Reflow global (nouveau) : fait tenir **toutes** les photos
  reflowFitAll: (preferCols='auto')=> set(s=>{
    const pg = s.pages[s.currentIndex];
    const photos = pg.items.filter((i:any)=>i.kind==='photo') as PhotoItem[];
    if (!photos.length) return s as any;
    ...
    const n = photos.length;
    const cols = (preferCols==='auto' ? clamp(Math.round(Math.sqrt(n)),1,4) : preferCols) as 1|2|3|4;
  ...
  loadFromSupabase: async (projectId)=>{
    ...
    const pages: Page[] = (ps||[]).map((p:any)=> ({ id:p.id, items: (its||[]).filter((it:any)=>it.page_id===p.id).map((it:any)=>it.data), preview:null }));
